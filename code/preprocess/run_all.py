...
for name, single_data in train_data.groupby('code'):
    tmp_data = single_data.drop(['time', 'code', 'value', 'label'], axis=1)
...
step = len(train_data) / 11
for ix in xrange(0, len(train_data), step):
    single_data = train_data.ix[ix:min(len(train_data)-1, ix+step-1), :]
    tmp_data = single_data.drop(['time', 'code', 'value', 'label'], axis=1)
    features = tmp_data.values.tolist()
    values = single_data['value'].tolist()
    labels = single_data['label'].tolist()
