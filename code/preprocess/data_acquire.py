    ...
    def run(self):
        ...
        while True:
            ...
            for ix in xrange(0, len(single_stock_data) - config.before - config.after, config.interval):
                ...
                features.extend([single_stock_data.loc[ix, 'time'], single_stock_data.loc[0, 'code'], after_price])
            ...
            print single_stock_data.loc[0, 'code'], len(single_stock_data), len(features_and_result_train), len(features_and_result_test)
...
def query(sql_list):
    ...
    for tmp, single_stock_data in stocks_data.groupby('code'):
