...
while ix < len(lines):
    ...
    if 'learning_rate' in line:
        ...
        strategy = tmp[0].split()[-1]
        punishment = float(tmp[0].split()[-1])
        ...
        ix += 4
        val_acc = float(lines[ix].split()[1]) / 100
        ix += 2
        test_acc = float(lines[ix].split()[1]) / 100

        single_result = result_template.get_new_result(
